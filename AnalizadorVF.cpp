...
using std::strncmp;
 ...
 string PalabrasReservadas[23] = {"si","entonces","sino","entero","decimal","logico","texto","char","mientras","haga","repita",
...
{
   string entrada;
   printf("Ingrese un archivo del lenguaje: ");
   cin >> entrada;
   ifstream archivo (entrada.c_str());
   string p;
   int i = 0;
   system("COLOR 0B");

   char palabra[1000]; //Esta variable es muy importante, pues guardara caracter por caracter formando asi una palabra y termina de guardar cuando detecta un espacio.
   int indexPalabra=0;
   int indexSimbolos = 0;
   ...
   while(getline(archivo, p) )
   {
   	int longitud = p.size();
	while(longitud > i )
	{
		if((p[i] != ' ') && (p[i] !=','))  	//Primero revisa que sea distinto de un espacio ' ' y una coma ','
      {
      	if((p[i]== ';') || (p[i]=='"') || (p[i]=='(') || (p[i]==')') || (p[i]=='+') || (p[i]=='-') || (p[i]=='*') || (p[i]=='/') || (p[i]=='#') || (p[i]=='<') || (p[i]=='>') || (p[i]=='='))
	      {
      	   tokenSimbolos[indexSimbolos] = p[i];
         	indexSimbolos++;
	      }
         else
         {
			palabra[indexPalabra] = p[i];
            auxWord[indexPalabra] = p[i];	//La razon de usar este arreglo es para mandar como parametro al metodo verificarNumero()... debido a un problema de programacion.
            indexPalabra++;
         }
      }
      else if((p[i] ==' ') || (p[i] ==','))
...
void imprimirIdentificadoresNoValidos(){
	...
	printf("********************************* MENSAJES *********************************\n");
   if(auxTNV == 0){
   	cout<<" 0 Errores, programa fuente exitoso...";
   }else{
   	for(int i=0;i<auxTNV;i++)
	{
   		cout<<"ERROR: No es Valido el Token:  \n"<<tokensNoValidos[i];
   	}
   }
}
...
bool verificarReservada(char palabra[1000]){
	int comp;
